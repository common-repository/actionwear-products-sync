 ...
 */
abstract class Action_Wear_Background_Process extends Action_Wear_Async_Request
{
    ...
    protected function lock_process()
    {
        ...
        $lock_duration = (property_exists($this, 'queue_lock_time')) ? $this->queue_lock_time : 180; // 2 minutes
    ...
    protected function handle()
    {
        ...
        do {
            ...
            foreach ($batch->data as $key => $value) {
                ...
                // delete because somebody requested task cancellation
                if (get_option("delete_" . $batch->key)) {
                    delete_option("delete_" . $batch->key);
                    $this->delete($batch->key);
                    break;
                }
    ...
    {
        $finish = $this->start_time + apply_filters($this->identifier . '_default_time_limit', 170); // 20 seconds
        $return = false;
